...
{
    private GameObject recordObject, startPoint, endPoint;
    ...
    // 書き込みなしコンストラクタ
    public FileOperation(string readFileName, int fileRowCount, GameObject startPoint, GameObject endPoint)
    {
    ...
    // 読み込みなしコンストラクタ
    public FileOperation(string writeFileName, GameObject recordObj, GameObject startPoint, GameObject endPoint)
    {
    ...
    // 読み書きありコンストラクタ
    public FileOperation(string readFileName, int fileRowCount, string writeFileName, GameObject recordObj, GameObject startPoint, GameObject endPoint)
    {
        this.recordObject = recordObj;
        this.startPoint = startPoint;
        this.endPoint = endPoint;
    ...
    public void RecordingUpdate()
    {
        ...
        if (interactUI && !startFlag && Vector3.Distance(RightHandPosition, startPoint.transform.position) < 0.1f) // startPoint付近に入ったとき
        {
            time = 0f;
            startFlag = true;
        }
