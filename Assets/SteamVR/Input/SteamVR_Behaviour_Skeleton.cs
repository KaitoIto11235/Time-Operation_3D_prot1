    ...
    public class SteamVR_Behaviour_Skeleton : MonoBehaviour
    {
        ...
        */
        ...
        protected virtual void UpdatePose()
        {
            ...
            skeletonPosition = ControllerOffsetPos(skeletonPosition, skeletonRotation);
            skeletonRotation = ControllerOffsetRot(skeletonPosition, skeletonRotation);
            //WristR.transform.position = skeltonPositionがしたい
            this.transform.position = this.transform.position + skeletonPosition - WristR.transform.position;
            //WristR.transform.rotation = skeletonRotationがしたい
            //1 = WristQ^(-1) * skQ
            //thisQ = thisQ * WristQ^(-1) * skQ
            this.transform.rotation = this.transform.rotation * Quaternion.Inverse(WristR.transform.rotation) * skeletonRotation;

            if (onTransformUpdated != null)
                onTransformUpdated.Invoke(this, inputSource);
        }

        public static Vector3 ControllerOffsetPos(Vector3 pos, Quaternion rotQ)
        {
            // コントローラの前方向を計算
            ...
            // シフト方向を決める重み
            float downWeight = 0.018f;
            float leftWeight = 0.042f;
            float forwardWeight = 0.15f;
            Vector3 dir = down * downWeight + left * leftWeight + forward * forwardWeight;
            
            // 前方向に沿って少し手前にオフセット
            float offsetDistance = -1f; // 手前に移動する距離（調整可能）
            Vector3 offset = dir * offsetDistance;

            return pos + offset;
        }

        public static Quaternion ControllerOffsetRot(Vector3 pos, Quaternion rotQ)
        {
            // コントローラの前方向を計算
            Vector3 forward = rotQ * Vector3.forward;
            // コントローラの下方向を計算
            ...
            Quaternion offsetRot = Quaternion.FromToRotation(forward, dir);
            return offsetRot * rotQ;
        }
